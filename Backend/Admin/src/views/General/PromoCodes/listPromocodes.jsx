...
const ListPromocodes = () => {
  ...
  const fetchUsers = async (page = 1, size = pageSize) => {
    try {
      const response = await fetch(`http://localhost:8000/api/users/getPromocodes?page=${page}&limit=${size}`);
  ...
  const exportToPDF = () => {
    ...
    const tableColumn = ['ID', 'promo_code', 'discount', 'discount_type', 'expiration'];
    const tableRows = listusers.map((row) => [row.id , row.promo_code, row.discount, row.discount_type, row.expiration]);
              ...
              <>
                <div className="d-flex justify-content-between align-items-center mb-3">
                  ...
                  <div className="d-flex">
                    ...
                    <Button onClick={() => navigate('/PromoCodes/AddNewPromocodes')} className="btn-dark">
                      Add New Promocode
                    </Button>
                  </div>
                </div>
