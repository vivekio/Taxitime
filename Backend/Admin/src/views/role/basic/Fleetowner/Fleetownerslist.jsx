...
const FleetownersList = () => {
  const navigate = useNavigate();
  const [listfleetsowner, setlistfleetsowner] = useState('');
...
const fetchUsers = async (page = 1, size = pageSize) => {
    try {
      const response = await fetch(`http://localhost:8000/api/users/listfleetsowner?page=${page}&limit=${size}`);
      const data = await response.json();

      setlistfleetsowner(data.fleets);
      setTotalRows(data.totalfleets); // Set total users count from API response
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        setlistfleetsowner(listfleetsowner.filter((user) => user.id !== id));
      } else {
  ...
  const handleSearch = (e) => {
    ...
    const filteredData = listfleetsowner.filter((user) =>
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="d-flex justify-content-start align-items-center">
          ...
          <button className="btn btn-danger btn-sm me-2 p-2" style={{backgroundColor: '#dc3545', borderColor: '#dc3545', color: '#fff'}} onClick={() => handleDelete(row.id)}>
  ...
  // 📌 Export to CSV
  const exportToCSV = () => {
    const csvData = listfleetsowner.map((row) => ({
      ID: row.id,
      Name: row.name,
      company: row.company,
      Email: row.email,
  ...
  // 📌 Export to Excel
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(listfleetsowner);
    const wb = XLSX.utils.book_new();
  ...
  const exportToPDF = () => {
    ...
    const tableRows = listfleetsowner.map((row) => [row.id, row.first_name, row.last_name, row.email, row.mobile]);
      ...
      <Row className="btn-page">
        <Col>
          <Card title="FLEET OWNERS">
            ...
            columns={columns}
            data={searchTerm ? filteredUsers : listfleetsowner}
            pagination
            paginationServer
...
export default FleetownersList;
