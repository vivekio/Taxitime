...
const DispatchersList = () => {
  ...
  const [listdispatchers, setlistdispatchers] = useState('');
  ...
  const fetchUsers = async (page = 1, size = pageSize) => {
    try {
      const response = await fetch(`http://localhost:8000/api/users/listdispatchers?page=${page}&limit=${size}`);
      const data = await response.json();

      setlistdispatchers(data.dispatchers);
      setTotalRows(data.totaldispatchers); // Set total users count from API response
    } catch (error) {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        setlistdispatchers(listdispatchers.filter((user) => user.id !== id));
      } else {
  ...
  const handleSearch = (e) => {
    ...
    const filteredData = listdispatchers.filter((user) =>
  ...
  // 📌 Export to CSV
  const exportToCSV = () => {
    const csvData = listdispatchers.map((row) => ({
      ID: row.id,
      Name: row.name,
      Email: row.email,
      mobile: row.mobile
  ...
  // 📌 Export to Excel
  const exportToExcel = () => {
    const ws = XLSX.utils.json_to_sheet(listdispatchers);
    const wb = XLSX.utils.book_new();
  ...
  const exportToPDF = () => {
    ...
    const tableRows = listdispatchers.map((row) => [row.id, row.name, row.email, row.mobile]);
            ...
            <OverlayTrigger>
              <>
                ...
                <DataTable
                  ...
                  columns={columns}
                  data={searchTerm ? filteredUsers : listdispatchers}
                  pagination
                  paginationServer
