...
const BasicButton = () => {
 ...
 const fetchUsers = async (page = 1, size = pageSize) => {
    try {
      const response = await fetch(`http://localhost:8000/api/users/getwaitingrequest?page=${page}&limit=${size}`);
    ...
    <React.Fragment>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Footer>
          ...
          <Button variant="dark" onClick={() => handleApprovebtn(listrequest[0].request_id)}>
            Approve
          </Button>
        </Modal.Footer>
      </Modal>
